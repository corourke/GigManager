...
- [ ] **0.1** Create a test gig. Gigs → **New Gig** (or **Add Gig**). Title it `SMOKE — Gig A`,
      give it any dates, status **Booked**. Save.
- [ ] **0.2** Create a second gig the same way, titled `SMOKE — Gig B`.
- [ ] **0.3** Open **SMOKE — Gig A**, scroll to the **Financials** section. Click **Edit
...
- [ ] **0.4** Do the same on **SMOKE — Gig B**: Contract Signed **$1,000**. Baseline: Revenue
      $1,000, Costs $0, Profit $1,000.
- [ ] **0.5** Create a manual purchase with two line items:
...
- [ ] Financials → **Purchases** tab. Expand the **`Van rental`** row (click it). In the line
      detail, use the **Assign Gig** picker and choose **SMOKE — Gig A**.
- [ ] A dialog **"Create gig expense record?"** appears, mentioning **$200.00** and Gig A.
      Click **Create record**.
- [ ] Toast confirms the entry was created.
- [ ] Open **SMOKE — Gig A** → Financials. **Costs** now reads **$200.00** and **Profit**
...
- [ ] Back on **Purchases**, expand `Van rental` again. Change the **Assign Gig** picker from
      Gig A to **SMOKE — Gig B**.
- [ ] Toast reads approximately **"Moved to SMOKE — Gig B — its ledger entry moved too"**
      (a *move*, not a new prompt to create a record).
- [ ] Open **SMOKE — Gig A** → Financials. **Costs** is back to **$0.00**, **Profit** back to
      **$1,000.00**. The `Van rental` expense row is **gone** from Gig A.
- [ ] Open **SMOKE — Gig B** → Financials. **Costs** reads **$200.00**, **Profit** **$800.00**,
...
- [ ] On **Purchases**, expand `Van rental`. In the **Assign Gig** picker, click the **✕ / clear**
      control to unassign the gig.
- [ ] A confirmation dialog **"Remove gig association and its ledger entry?"** appears,
      mentioning **$200.00**. Click **Keep linked** (cancel it).
- [ ] Nothing changed: the line is still assigned to Gig B; **SMOKE — Gig B** Costs still
      **$200.00**.
- [ ] Clear the gig again — this time click **Unlink and delete entry**.
- [ ] Toast confirms removal. Open **SMOKE — Gig B** → Financials: **Costs** back to **$0.00**,
      **Profit** back to **$1,000.00**, the `Van rental` row is gone.
- [ ] Back on Purchases, the `Van rental` line shows no "Gig Details" button (fully unlinked).
...
- [ ] Starting from the unlinked `Van rental` line: assign it to **SMOKE — Gig A**, click
      **Create record** on the prompt. Gig A **Costs $200.00**.
- [ ] Reassign the line to **SMOKE — Gig B** (move). Gig A **Costs $0.00**, Gig B **Costs $200.00**.
- [ ] Reassign the line back to **SMOKE — Gig A** (move). Gig A **Costs $200.00**, Gig B **Costs $0.00**.
- [ ] Clear the gig → **Unlink and delete entry**. Gig A **Costs $0.00**.
- [ ] Assign it to **SMOKE — Gig A** once more → **Create record**.
- [ ] **Final check:** **SMOKE — Gig A** Financials shows **Costs $200.00**, **Profit $800.00**,
...
- [ ] Leave `Van rental` assigned to **Gig A** with its ledger entry — Part 6 uses it.
...
- [ ] Open **SMOKE — Gig A** → Financials. Find the `Van rental` expense row. It has a
      **paperclip** icon button in the row's action area (visible even without Edit mode).
- [ ] With Edit mode **off**, click the paperclip. A **"Receipts & Documents"** dialog opens.
      It says "No attachments found" and shows **no Upload button** (view-only outside Edit
      mode).
- [ ] Close the dialog. Click **Edit Financials**. Click the paperclip on `Van rental` again —
      now an **Upload** button is present.
...
- [ ] First, make sure the receipt has an **unlinked** line: on **Purchases**, the `SMOKE
...
- [ ] In the `SMOKE Vendor` card **header row** (the grey bar with the vendor name and Invoice
      Total), there is an **"Assign receipt to gig…"** picker. Choose **SMOKE — Gig B**.
- [ ] Toast reads approximately **"Assigned receipt and its lines to SMOKE — Gig B"**.
- [ ] A dialog **"Add these expenses to the gig ledger?"** appears, naming a count of expense
      lines and Gig B. Click **Add all**.
- [ ] Toast confirms N lines added.
- [ ] Open **SMOKE — Gig B** → Financials. **Costs** increased by the total of the lines that
      were just linked:
      ...
      it's still on Gig A.
- [ ] Back on Purchases, the `SMOKE Vendor` header now shows a **"Gig Details"** button, and the
...
- [ ] On **Purchases**, click **Add New**. Vendor `SMOKE Vendor 2`, Invoice Total `50`, one
      item: `Parking`, qty 1, price `50`. **Save Purchase**.
- [ ] Expand the `Parking` line. In **Assign Gig**, choose **SMOKE — Gig A**.
- [ ] On the **"Create gig expense record?"** prompt, click **Skip** this time.
- [ ] The line is now linked to Gig A (it has a "Gig Details" button) but the line detail also
      shows a green **"Add to gig ledger"** button — because the ledger entry was skipped.
- [ ] Confirm the skip really left the ledger alone: **SMOKE — Gig A** Financials **Costs** is
      unchanged (still whatever Part 1/4 left it at — no +$50).
- [ ] Back on Purchases, expand `Parking`, click **"Add to gig ledger"**. Toast confirms.
- [ ] The **"Add to gig ledger"** button is now **gone** from that line.
- [ ] **SMOKE — Gig A** Financials → **Costs** increased by **$50.00**, and a `Parking`
      expense row is present.
- [ ] Expand `Parking` again — clicking the assign picker's value shows it's still Gig A, and
...
- [ ] Go to the **Asset List** screen → **Import** → select type **Assets** → **Download
      Template** if you want the column layout.
- [ ] Prepare a tiny CSV with one **expense** row: `source` = `2` (Expense), an
...
- [ ] Upload it, import the valid row.
- [ ] Go to **Financials → Purchases**. There is a new `SMOKE Import` card with a `Gaffer tape`
      expense line (possibly under a "synthesized" header). It has **no gig** and **no** ledger
      entry anywhere.
- [ ] Expand the `Gaffer tape` line → **Assign Gig** → **SMOKE — Gig B**. (You may or may not
      get the "Create gig expense record?" prompt here — either **Skip** it, or if there's no
      prompt, that's expected for imported lines.)
- [ ] The line detail shows the green **"Add to gig ledger"** button. Click it.
- [ ] **SMOKE — Gig B** Financials → **Costs** increased by **$75.00**, `Gaffer tape` row
...
- [ ] Open **SMOKE — Gig A** → Financials → **Edit Financials**. On the `Van rental` row click
      the **trash / delete** icon. Confirm.
...
- [ ] Attach a PDF to any remaining expense on **SMOKE — Gig B** (paperclip, in Edit mode).
- [ ] Delete **SMOKE — Gig B** entirely (gig detail → the delete/remove action, confirm).
- [ ] No error. Go to **Financials → Purchases**: the purchase lines that had been linked to
