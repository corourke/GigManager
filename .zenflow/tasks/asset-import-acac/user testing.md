...
- [x] **Upload Invoice**: Click **Upload Invoice** on the Asset List screen. Select a PDF invoice.
- [x] **Extraction Pulse**: Verify the loading/scanning state appears while the AI processes the file.
- [x] **Review Dialog**: Verify the `ReviewScannedDataDialog` opens with extracted Vendor, Date, and Total.
- [x] **Line Item Verification**: Check that individual line items (Manufacturer/Model, Qty, Price) are correctly extracted.
- [x] **Reconciliation Status**: Verify the "Reconciliation Successful" message if line items sum to the total, or a warning if they don't.
- [x] **Manual Adjustment**: Edit an extracted price or quantity. Verify the "Total Mismatch" warning updates in real-time.
- [x] **Persistence**: Click **Confirm and Save**. Verify the new Assets appear in the list with correct costs.
- [x] **Automatic Linking**: Open one of the newly created assets. Click the **Financials** or **Attachments** tab (if applicable) and verify the source invoice is linked.

#### **3. Gig Screen: Receipt Import**
- [x] **Section Visibility**: Navigate to a Gig and locate the **Purchase Expenses** section.
- [x] **Upload Receipt**: Use the **Upload Receipt** button. Verify it triggers the same AI extraction flow.
- [x] **Gig Association**: After saving, verify the Purchase header appears in the Gig's expense list.
- [x] **Detail View**: Click the **File** icon on a linked expense. Verify the `AttachmentManager` appears below the table, showing the original receipt.
- [x] **Unlink Expense**: Click the **Trash** icon to unlink. Verify the expense disappears from the Gig but remains in the global Financials/Purchases list.

#### **4. Backend Logic & Allocation**
- [x] **Cost vs. Price Path**: Test an invoice where some items have a "Price" and others have a "Cost". Verify that `item_cost` is calculated correctly for all items based on the invoice total (including tax/shipping allocation).
- [x] **Penny Reconciliation**: Use an invoice total like $100.00 with 3 items at $33.33. Verify the 3rd item is adjusted to $33.34 to ensure the total matches exactly.
- [x] **Data Propagation**: Verify that items created via the AI pipeline correctly inherit the `vendor` and `acquisition_date` from the invoice header.
...
- [x] **A-Z Template**: Generate and download the Asset Template. Verify it contains the new columns (A-Z mapping).
- [x] **Grouped Import**: Upload a CSV with a Source 0 (Header) followed by Source 1 (Asset) rows. Verify that the assets are correctly grouped and their costs are burdened by the header's total.
